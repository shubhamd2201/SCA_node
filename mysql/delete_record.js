...
app.delete('/api/deletestudentdata/:roll_n', (req, res)=>{

    let roll_no = req.body.roll_no;

        connection.query('delete from students where roll_no=?', [roll_no], (err, results)=>{
            if(err){
                return res.status(500).send(`Error in Deleting data ${err}`);
            }
            if(results.affectedRows === 0){
                return res.status(400).send({error:`record of Roll no ${roll_no} not found`});
            }
            console.log(results)
            res.send({message: "student deleted", data:roll_no});
            return;
        });
});
